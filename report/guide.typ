...
#let source = s => {
  block(width: 100%)[#{
    ...
    v(-0.5em)
    pad(left: 5%)[#block(fill: rgb("f5f5f5"), width: 100%, inset: 10pt)[#raw(
      s,
      lang: "typ",
    )]]
    set align(center)
...
#let typst-rendering(d) = {
  ...
  pad(left: 5%)[#block(fill: rgb("f5f5f5"), inset: 10pt, width: 100%)[#eval(
    d,
    mode: "markup",
  )]]
  source(d)
...
// Cover page
#template.cover(title, author, date-start, date-end, subtitle: subtitle, logo: logo, logo-horizontal: logo-horizontal)
#pagebreak()
```
...
#pagebreak()
#show: template.appendix.with(title: "Appendix")
